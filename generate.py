...
#MAGAZYN.BULK
f = open("dane_do_generowania/magazyn_nazwa_i_adres.txt", "r")
magazyn_bulk=open("bulks/magazyn.bulk", "w")
counter = 0
id = 0
liczba_magazynow=0
for line in f:
...
#FABRYKA BULK
f = open("dane_do_generowania/fabryki_nazwa_i_adres.txt", "r")
fabryka_bulk=open("bulks/fabryka.bulk", "w")
liczba_fabryk=0
for line in f:
    fabryka_bulk.write(str(counter+1)+"|"+line.split(' ')[0]+"|"+" ".join(str(x) for x in line.split()[1:])+"\n")
    counter=counter+1
    id = id + 1
    if counter>9:
        counter=0
...
agent_bulk=open("bulks/agent.bulk", "w")
imiona_d = open('dane_do_generowania/imiona_damskie_polskie.txt',encoding="utf8").read().splitlines()
imiona_m = open('dane_do_generowania/imiona_meskie_polskie.txt',encoding="utf8").read().splitlines()
nazwiska_d = open('dane_do_generowania/nazwiska_damskie_polskie.txt',encoding="utf8").read().splitlines()
nazwiska_m = open('dane_do_generowania/nazwiska_meskie_polskie.txt',encoding="utf8").read().splitlines()
...
klient_bulk=open("bulks/klient.bulk", "w")
miasta = open('dane_do_generowania/ludnosc_do_miasta.txt',encoding="utf8").read().splitlines()
ulice = open('dane_do_generowania/ulice_polskie.txt',encoding="utf8").read().splitlines()
print(miasta[1].split(',')[1])
...
#PRODUKT BULK
produkt_bulk=open("bulks/produkt.bulk", "w")
nazwy_produktow = open('dane_do_generowania/nazwy_produktow.txt',encoding="utf8").read().splitlines()
...
for id in range(len(nazwy_producentow)*len(nazwy_produktow)):
    produkt_bulk.write(str(id+1)+"|"+nazwy_produktow[id%len(nazwy_producentow)]+"|"+str(random.randint(15,1000))+"."+str(random.randint(0,99))
...
#KURIER_BULK
kurier_bulk=open("bulks/kurier.bulk", "w")
nazwy_kurierow = open('dane_do_generowania/nazwy_kurierow.txt',encoding="utf8").read().splitlines()
...
#ZAMOWIENiE_BULK
zamowienie_bulk=open("bulks/zamowienie.bulk", "w")
...
prod_do_zam_bulk=open("bulks/produkt_do_zamowienia.bulk", "w")
for id in range(liczba_zamowien):
    produkty=[]
...
#SPRAWA BULK
sprawa_bulk=open("bulks/sprawa.bulk", "w")
liczba_spraw=10000
szczeg_zam = open('bulks/zamowienie.bulk', encoding="utf8").read().splitlines()
...
#ROZMOWA BULK
rozmowa_bulk=open("bulks/rozmowa.bulk", "w")
print("Ile ma byc faktow?")
szczeg_sprawy = open('bulks/sprawa.bulk', encoding="utf8").read().splitlines()
#liczba_faktow = input()
liczba_faktow = 50000
